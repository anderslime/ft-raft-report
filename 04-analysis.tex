...
One important thing to make this approach work well is to derive the bevaviour that seem most easy to implement first.

% subsection our_approach (end)

% Vores approach til udvikling
    % - Iterativ
    % - Ud fra opførsel specificeret i "the raft paper" - Behaviour Driven Development
    % - BDD:
    %   - Behaviour ->  Faililing Test (specificatation)
    %   - Implement until tests passes
    %   - Refactor
    %   - Repeat

%Hvordan kommer vi fra requirements til en implementation?

%Hvordan har vi tænkt os at implementere? waterfall vs. iterativ

%Hvordan vil vi verificere programmet (scenarie tests)?
